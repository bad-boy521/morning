...
class NightScreen(Screen):
    ...
    def setup_background(self):
        night_images_path = 'assets/images/night'
        background_images = [f for f in os.listdir(night_images_path)
                             if f.endswith(('.jpg', '.png'))]
        if background_images:
            random_image = random.choice(background_images)
            background_path = os.path.join(night_images_path, random_image)
    ...
    def get_weather_text(self):
        try:
            ...
            return f"[size=24sp]今日天气回顾[/size]\n{weather_info}"  # 使用标记语言设置大标题
